                ...
                for _, current_image in ipairs(images) do
                    if current_image.window then
                        ...
                        if current_image.buffer then
                            ...
                            local is_buffer_in_window = vim.api.nvim_win_get_buf(current_image.window) ==
...
---@param id string
api.enableOnly = function(id)
    state.enabled = true
    local images = api.get_images()
    for _, current_image in ipairs(images) do
        if id == current_image.id then
            current_image:render()
        end
    end
end
